...
alias fe="fzf --preview 'fzf-preview {}' | xargs -r nvim"
# alias fi="fzf --preview 'fzf-preview {}' | xargs -r nomacs"
alias fv="fzf --preview 'fzf-preview {}' | xargs -r vlc"

# Code
alias vcore='code --new-window ~/git/veloce/core/ && exit' 
alias vui='code --new-window ~/git/veloce/ui/ && exit' 
alias vnix='code --new-window ~/git/veloce/nix-os/ && exit' 
alias vbase='code --new-window ~/git/veloce/base/ && exit' 
alias vsim='code --new-window ~/git/veloce/sim/ && exit' 
...
share_internet() {
  ...
  # bring link up
  sudo ip link set dev "$ETH_IF" up || { echo "ERROR: failed to bring $ETH_IF up"; return 5; }
  ...
  sudo iptables -t nat -A POSTROUTING -o "$OUT_IF" -j MASQUERADE \
    || { echo "ERROR: failed to add POSTROUTING MASQUERADE"; return 6; }

  sudo iptables -A FORWARD -i "$OUT_IF" -o "$ETH_IF" -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT \
    || { echo "ERROR: failed to add FORWARD for established connections"; return 7; }

  sudo iptables -A FORWARD -i "$ETH_IF" -o "$OUT_IF" -j ACCEPT \
    || { echo "ERROR: failed to add FORWARD from $ETH_IF to $OUT_IF"; return 8; }
